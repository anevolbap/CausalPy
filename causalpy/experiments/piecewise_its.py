...
class PiecewiseITS(BaseExperiment):
    ...
    **With datetime thresholds:**
    ...
    def get_plot_data(self, hdi_prob: float = HDI_PROB) -> pd.DataFrame:
        ...
        data["effect"] = effect.mean(dim=["chain", "draw"]).values
        if with_uncertainty:
        ...
        data["cumulative_effect"] = cumulative_effect.mean(dim=["chain", "draw"]).values
        if with_uncertainty:
            cum_lower, cum_upper = hdi_bound_arrays(cumulative_effect, prob=hdi_prob)
